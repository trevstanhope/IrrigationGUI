...
function [eto] = ETO_ASCE(Tmin,Tmax,u2,Rs,Ra,z,C)
% Initializatiaons
eo_Tmax = zeros(1,12);
eo_Tmin = zeros(1,12);
es = zeros(1,12);
...
% Switch to 0.38 if no CO2 increase
if (C == 0.34)
    C = 0.38;
end
...
for i=1:12
    ...
    if i>11
        ...
        eto(i)=((0.408*delta*(Rn(i)-G(i)))+(phi*(1600/(Tmean(i)+273))*u2(i)*(es(i)-ea(i))))/(delta+(phi*(1+(C*u2(i)))));
